...
async function registerController(req, res) {
  try {
    const { firstName, lastName, email, password, phoneNumber, address, city, postalCode, country, region } = req.body;
    const encryptedPassword = await bcrypt.hash(password, 10);
    const newUser = await User.create({ firstName, lastName, email, password: encryptedPassword, phoneNumber, address, city, postalCode, country, region })
